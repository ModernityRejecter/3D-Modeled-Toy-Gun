...
A fully 3D-printable, **hammer-fired** mechanical toy blaster built around a two-piece press-fit case, a snap-locking side magazine, and **three entirely 3D-printed springs**. No metal hardware, no rubber bands - every functional part, including all springs, comes off the print bed.
...
The blaster's case is a **two-piece clamshell**, split along the centerline of the barrel into an **upper** and a **lower** half. Each half carries similar, mirrored features - half the barrel bore, the grip, and the internal cavities - and together they enclose the hammer, trigger and springs. The halves are held shut by **two snug press fits**, and a **detachable magazine** mounts on the side.
...
- The **two-piece clamshell case** (split along the barrel centerline) lets the whole mechanism print support-free and gives full assembly access to the hammer, trigger and springs; the two halves then close over the internals with two snug press fits - no screws or glue.
...
|:---:|---|---|---|
| <img src="UpperCaseRender.png" width="150"> | **Upper case** | One half of the clamshell case (split along the barrel centerline). Carries half the barrel bore, grip and internal cavities, and encloses the hammer, trigger and springs together with the lower half. | `UpperCase.gcode` |
| <img src="LowerCaseReneder.png" width="150"> | **Lower case** | The matching case half, with the mirrored half of the barrel, grip and trigger guard. Holds the hammer and trigger springs and accepts the side magazine; mates to the upper half via two snug press fits. | `LowerCase+TorsionSprings.gcode` |
...
- **Press-fit + snap-fit assembly** - the two case halves hold together with two snug press fits and the magazine snap-locks to the case; no screws or glue required.
...
**Tools used:** Autodesk Fusion 360 (modeling, joints, motion study) · PrusaSlicer (G-code generation) · Google Gemini (design sounding-board for evaluating the feeding and magazine-catch options) · Claude (Anthropic) (structuring and polishing this README).
...
- **Making the hammer action and magazine feed coexist** was the core mechanical problem - the hammer had to strike and clear the chamber on every cycle without the follower spring fouling its travel, which is what drove the internal layout and the placement of the three springs.
